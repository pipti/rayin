...
2019.8  构思设计、技术调研分析  
2019.9  基础合成模块研发  
2019.10 结合项目进行合成测试以及优化调整  
2019.11 应用场景分析设计
2019.11 itext2版本
2019.12 itext5版本 
2020.01 原型设计、后台搭建  
2020.02 模板设计、富文本插件
2020.06 openhtmltopdf版本
...
1. 实现html转换PDF.
2. 实现json数据模板合成PDF.
3. 以下罗列为openhtmltopdf支持的CSS生成电子文件常用的特性：
    1. 支持自动扩展，包括表格，长文本.
    2. 支持表格头尾分页保留头尾.
    ...
    6. 支持目录标签.
    7. 实现二维码和条形码的显示.
4. 以下罗列thymeleaf动态数据绑定生成电子文件常用的特性：
    1. 动态数据.
    2. 数据嵌套循环.
    3. 显示块整体循环，table，div.
    4. 数据格式化.
    ...
    6. 国际化.
5. 实现构件配置组合，构件组合模板.
6. 实现可依据模板的配置文件或动态json配置数据进行生成.
7. 实现构件对应不同页尺寸组合，例如A4+B5+A3.
8. 实现构件类型自定义配置（主要用于后续大型打印机的纸盒识别，可通过模板配置后，绑定至PDF对应的元数据中，通过读取再加工转换为PS文件指定至对应的纸盒）.
9. 实现构件空白页填充（奇数页构件补空白页）.
10. 实现页码灵活计数，页码灵活显示，支持构件重计数,单页显示多页码.
11. 支持生僻字显示.
12. 实现pdf元数据设置.
13. 实现pdf嵌入.
14. 实现字体自动加载.
15. 实现pdf文件信息的源数据设置和读取.
