...
export default {
  ...
  data () {
    return {
      ...
      init: {
        ...
        content_style: '',
        content_css: `/static/tinymce/editor-content-customize.css`,
        font_formats: '',
        ...
        init_instance_callback: (editor) => {
          ...
          //          })
          let initEditorValue = this.initEditorValue
          ...
          let tinymceActiv = tinymce.activeEditor
          let tinymceActivBody = tinymceActiv.getBody()
          tinymceActiv.selection.select(tinymceActivBody)
          let text = tinymceActiv.selection.getContent({ 'format': 'text' })
          // console.log('editor中纯文本内容' + text.charCodeAt() + '---')
          if (text.charCodeAt() === 10) {
            axios.get('/static/tinymce/init.html',
              {},
              {})
              .then(res => {
              // 初始化编辑器中的HTML
  ...
  mounted () {
    axios.get(this.GLOBAL.webappApiConfig.ElementDesign.UserElementFonts.url,
      {},
      {})
      .then(res => {
        if (res.data.code === 0) {
